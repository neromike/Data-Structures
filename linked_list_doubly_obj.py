class linked_list_doubly_obj(object):
	...
	class node_obj(object):
		"""The node for each item contains a link to the "next" node, a link to the "prev" node, as well as a "data" payload. If no data is passed, then payload is set to None."""
	...
	def add_to_head(self, this_data):	#O(1)
		...
		this_node = linked_list_doubly_obj.node_obj(this_data)
		if self.root != None:	#edge case = empty linked list
	...
	def add_to_tail(self, this_data):	#O(n)
		...
		this_new_node = linked_list_doubly_obj.node_obj(this_data)
		this_node = self.root
		if this_node == None:	#edge case = empty linked list so pass data will be root
...
import unittest
class test_linked_list_doubly(unittest.TestCase):
	def setUp(self):
		self.this_list = linked_list_doubly_obj()
